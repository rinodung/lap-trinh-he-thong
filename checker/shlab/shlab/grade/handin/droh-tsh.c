...
void eval(char *cmdline) 
{
	...
	/* This is a little tricky. Block SIGCHLD signals until we can
	add the job to the job list.  This ensures that the call to
	addjob() in the parent runs before the call to deletejob() in
	the sigchld handler. Avoids a race condition between the
	parent and child, where the child terminates before the parent
	ever gets a chance to run after the fork. */
	...
	if (pid == 0) {
	    /* Child unblocks SIGCHLD signals */
	...
	/* Parent adds the job, and then unblocks SIGCHLD so that
	   the sigchld handler can run again */
