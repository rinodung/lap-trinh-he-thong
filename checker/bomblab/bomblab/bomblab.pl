...
#
# Copyright (c) 2003-2016, R. Bryant and D. O'Hallaron, All rights
...
while (1) { 
    ...
    if ($found_requestd != 1) {
        log_msg("Detected $found_requestd instances of $Bomblab::REQUESTD");
    ...
    if ($found_resultd != 1) {
        log_msg("Detected $found_resultd instances of $Bomblab::RESULTD");
    ...
    if ($found_reportd != 1) {
        log_msg("Detected $found_reportd instances of $Bomblab::REPORTD");
