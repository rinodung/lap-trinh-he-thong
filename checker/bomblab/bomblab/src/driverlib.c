			...
			char *status_msg)  /* Status message returned to caller */
{
    ...
    if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
		strcpy(status_msg, "Error: Client unable to read status message from server");
		close(clientfd);
		return -1;
    }
 ...
 */
int driver_post(char *userid, char *user_pwd, char *result, int autograded, char *status_msg)
{
    int status;
